...
! (function($, window, document, undefined) {
	...
	/* Use as $("imgs:below-the-fold").something() or */
	/* $("imgs").filter(":below-the-fold").something() which is faster */
	$.extend($.expr[":"], {
	...
	var methods = {
		init: function(settings) {
			return this.each(function() {
				...
				self.stars = $this.children('imgs:not(".raty-cancel")');
				self.score = $('<input />', {
					type: 'hidden',
					...
					timer = setTimeout(function () {
						...
						if (bigImg == '') {
							...
							'<div id="preview-wraper" style="position: absolute;width:'+options.bigImgWidth+'px;height:auto;top:' + imgT + 'px;right:' + tooltipRight + ';left:' + tooltipLeft + '">'+
								'<imgs src="'+smallImg+'" width="'+options.bigImgWidth+'">'+
							'</div>';
							...
							image.onload = function() {
								$('#preview-wraper').find("imgs").attr("src",bigImg).css("width",options.bigImgWidth);
							};
						}
					},500);
				},
